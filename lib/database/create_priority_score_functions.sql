-- ============================================
-- Priority Score Functions - Fixed Version
-- ============================================
...
-- ============================================
-- 3. Seasonal Score Function (with date parameter)
-- ============================================
CREATE OR REPLACE FUNCTION calculate_seasonal_score_with_date(product_id uuid, current_date date)
RETURNS integer AS $$
DECLARE
  season_start date;
  season_end date;
...
-- ============================================
-- 3b. Seasonal Score Function (default to current date)
-- ============================================
CREATE OR REPLACE FUNCTION calculate_seasonal_score(product_id uuid)
RETURNS integer AS $$
BEGIN
  RETURN calculate_seasonal_score_with_date(product_id, CURRENT_DATE);
END;
$$ LANGUAGE plpgsql STABLE;

-- ============================================
-- 4. Festival Score Function (with date parameter)
-- ============================================
CREATE OR REPLACE FUNCTION calculate_festival_score_with_date(product_id uuid, current_date date)
RETURNS integer AS $$
DECLARE
  festival_date date;
  days_before integer;
...
-- ============================================
-- 4b. Festival Score Function (default to current date)
-- ============================================
CREATE OR REPLACE FUNCTION calculate_festival_score(product_id uuid)
RETURNS integer AS $$
BEGIN
  RETURN calculate_festival_score_with_date(product_id, CURRENT_DATE);
END;
$$ LANGUAGE plpgsql STABLE;

-- ============================================
-- 5. Ingredient Expiry Score Function (with date parameter)
-- ============================================
CREATE OR REPLACE FUNCTION calculate_ingredient_expiry_score_with_date(product_id uuid, current_date date)
RETURNS integer AS $$
DECLARE
...
-- ============================================
-- 5b. Ingredient Expiry Score Function (default to current date)
-- ============================================
CREATE OR REPLACE FUNCTION calculate_ingredient_expiry_score(product_id uuid)
RETURNS integer AS $$
BEGIN
  RETURN calculate_ingredient_expiry_score_with_date(product_id, CURRENT_DATE);
END;
$$ LANGUAGE plpgsql STABLE;

-- ============================================
-- 6. Total Priority Score Function (with date parameter)
-- ============================================
CREATE OR REPLACE FUNCTION calculate_priority_score_with_date(
  product_id uuid,
  margin_pct numeric,
...
BEGIN
  ...
  seasonal_score := calculate_seasonal_score_with_date(product_id, current_date);
  festival_score := calculate_festival_score_with_date(product_id, current_date);
  ingredient_score := calculate_ingredient_expiry_score_with_date(product_id, current_date);
...
-- ============================================
-- 6b. Total Priority Score Function (default to current date)
-- ============================================
...
RETURNS numeric AS $$
BEGIN
  RETURN calculate_priority_score_with_date(product_id, margin_pct, days_remaining, CURRENT_DATE);
END;
$$ LANGUAGE plpgsql STABLE;
...
COMMENT ON FUNCTION calculate_seasonal_score(uuid) IS 'คำนวณคะแนนความเหมาะสมฤดูกาล: ในฤดู=100, ใกล้สิ้นฤดู<30วัน=80, นอกฤดู=0';
COMMENT ON FUNCTION calculate_festival_score(uuid) IS 'คำนวณคะแนนความเหมาะสมเทศกาล: วันเทศกาล=100, 1-7วัน=90, 8-14วัน=70, อื่น=0';
COMMENT ON FUNCTION calculate_ingredient_expiry_score(uuid) IS 'คำนวณคะแนนความเร่งด่วนวัตถุดิบ: ≤7วัน=100, 8-14วัน=70, อื่น=0';
COMMENT ON FUNCTION calculate_priority_score(uuid, numeric, integer) IS 'คำนวณคะแนนรวม Priority Score จาก 5 ปัจจัย';
