...
SELECT 
  ...
  COALESCE(od.discount_value, 0) as discount_value,
...
SELECT 
  ...
  o.id as order_id,
  o.order_number,
  o.net_total as order_total,
  o.net_total as order_final_amount,
  ...
  od.applied_at,
  od.applied_by,
  od.applied_by as applied_by_name,
...
SELECT 
  ...
  COALESCE(SUM(total_discount), 0) as discount_by_type,
  ...
  -- Calculations
  COALESCE(AVG(total_discount), 0) as avg_discount_per_usage,
  COALESCE(MAX(total_discount), 0) as max_discount_per_usage,
  COALESCE(MIN(total_discount), 0) as min_discount_per_usage
