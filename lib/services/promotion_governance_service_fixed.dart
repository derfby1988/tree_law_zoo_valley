...
class PromotionGovernanceService {
  ...
  static Future<List<Map<String, dynamic>>> getConflicts({
    ...
    try {
      var query = _client
          .from('promotion_conflicts')
          .select('*, promotion:pos_promotions(name)');
  ...
  static Future<List<Map<String, dynamic>>> getApprovalRequests({
    ...
    try {
      var query = _client
          .from('promotion_approvals')
          .select('*, promotion:pos_promotions(name), requester:requested_by(display_name)');

      if (status != null) {
        query = query.eq('status', status);
      }
      if (assignedTo != null) {
        query = query.eq('assigned_approver_id', assignedTo);
      }
  ...
  static Future<List<Map<String, dynamic>>> getAuditLogs({
    ...
    try {
      var query = _client
          .from('coupon_promotion_audit_log')
